 ...
 *
 * 「多磨霊園・周辺の読みもの」機能のデータ基盤（2026-09-13新設、2026-09-14に
 * 「多磨霊園・周辺の新着記事」から改称）。businesses.json/sources.csvや
 * sponsors.jsonとは完全に独立したファイル・読み込み経路とする（既存の比較・
 * スポンサー表示には一切影響しない）。
 *
 * 【公式情報／周辺の読みもの の役割分担】詳細はCLAUDE.md2章「公式情報と
 * 周辺の読みものの分離」を正とする。このデータは独自の地域メディア・
 * ブログ枠（周辺の飲食店・散策情報、季節の話題、歴史、著名人紹介、実用
 * 情報等）専用であり、東京都公園協会等の**公式発表・一次情報は入れない**
 * （公式発表はsrc/pages/_lib/tama-news.tsのTamaNewsItemで扱う。2026-09-14、
 * 実際に公式発表記事が一度ここへ混入したため、移設した経緯がある）。
 *
