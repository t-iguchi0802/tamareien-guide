...
- 条件未選択時はスポンサー契約の有無を通常掲載順へ反映しない。サービス条件検索時だけ、条件に一致するスポンサー契約店を「PR」表示付きで先頭表示する。比較内容・評価は変更しない。
...
- 条件未選択時はスポンサー契約の有無を通常掲載順へ反映しない。
- サービス条件が1つ以上選択された場合だけ、AND条件に一致するスポンサー契約店を「PR」と明記して結果の先頭へ表示する。条件に一致しないスポンサーは表示せず、同じ事業者を重複表示しない。
- 比較内容・確認状態・評価はスポンサー契約によって変更しない。独立した4枠の広告欄も従来どおり表示する。
...
### 公式情報と周辺の読みものの分離（2026-09-14、サイト所有者判断による恒久ルール）

本サイトのコンテンツは、性質の異なる次の2種類に必ず分離する。どちらか一方の
枠へ他方の内容を混ぜない（過去に一度、東京都公園協会の公式発表記事が
「周辺の読みもの」側のデータへ混入したため、本ルールを明文化する）。

**A. 公式情報**（対象: 多磨霊園、東京都、公益財団法人東京都公園協会、
関係自治体、交通機関、公共施設等の公式発表・一次情報）

- 利用者に必要な要点だけを短く要約する。公式発表を無理に長い独自記事へ
  加工しない。
- 必ず元の公式情報へのリンクを表示する。
- 事実のみを扱い、推測や感想を加えない。元ページ本文を長く転載しない。
- 「当サイトが公式である」という意味にならないようにする。
- 表示上は「多磨霊園の公式情報」または「多磨霊園の公式お知らせ」の表現を
  使う。
- 情報源名・公開日・公式リンクを明確にする。
- 実装: `src/pages/_lib/tama-news.ts`（`TamaNewsItem`）。TOPの
  「多磨霊園の公式お知らせ」セクションで表示する。**このデータには
  公式発表・一次情報以外を入れない。**

**B. 周辺の読みもの**（独自の地域メディア・ブログ枠。対象例: 墓参り後に
寄れる飲食店・喫茶店・和菓子店、多磨霊園周辺の散歩、季節の風景、多磨霊園の
歴史、著名人の墓所と人物紹介、駅から実際に歩くルート、混雑やアクセスの
実用情報、周辺の小さな発見、「墓参り＋◯◯」の半日コース等）

- 単なるニュース転載・要約にしない。読んで面白い、行ってみたくなる、
  役に立つ記事を優先する（地域メディア・ブログとしての独自性を持たせる）。
- 可能であれば、現地で確認すると面白くなるポイントも記事化する。
- 店舗の営業時間等の実用情報は公式情報を確認する。口コミのみで事実を
  断定しない。
- 著名人の墓所等、重要な事実は複数の信頼できる情報源で照合する。
- 事実と推測・感想を明確に分けて書く。
- 実装: `src/data/news.json`（`NewsArticle`、`src/types/news.ts`）。
  `/news/`（表示名称「多磨霊園・周辺の読みもの」）の一覧・詳細ページ、
  およびTOPの同名セクションで表示する。**このデータへ、公式発表の要約
  だけの記事を入れない。**（東京都公園協会等の公式発表はAの
  `tama-news.ts`側で扱う。）`/news/`のURLは名称変更後も既存リンクを
  維持するため変更しない。記事が0件でもビルド・表示が正常に成立する
  ことを維持する（空の枠・ダミー記事は表示しない）。

**共通ルール**

- 上記A・Bを混同しない。公式発表の要約記事を`news.json`（B）へ、逆に
  独自取材の読みものを`tama-news.ts`（A）へ入れない。
- 外部情報の自動収集から直接自動公開しない。ChatGPT等のAIで収集・下書きを
  作成してもよいが、公開前に必ず人が内容を確認してからデータへ追加する
  （無審査自動更新は本MVPの対象外という既存ルールと同じ考え方）。
