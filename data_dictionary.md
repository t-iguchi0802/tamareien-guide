...
6. 条件未選択時はスポンサー契約の有無を通常掲載順へ反映しない。サービス条件検索時だけ、条件に一致するスポンサー契約店を「PR」表示付きで先頭表示する。比較内容・評価は変更しない。
...
- サービス条件が1つ以上選択された場合だけ、AND条件に一致するスポンサー契約店を結果の先頭へ表示する。
- 条件未選択時は通常掲載順を維持し、不一致スポンサーは表示せず、同じ事業者を重複表示しない。
